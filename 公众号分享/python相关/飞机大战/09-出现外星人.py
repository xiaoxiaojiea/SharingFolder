...
class game_functions():
    ...
    # 屏幕更新函数: （屏幕，设置参数，飞船对象）
    def updata_screen(self,screen,mySettings,ship,bullets,alien):
...
class Alien(Sprite):
    ...
    def __init__(self, mySettings, screen):
        ...
        self.alienRect = self.image.get_rect()

        # 每个外星人最初都在屏幕左上角附近
        self.alienRect.x = self.alienRect.width
        self.alienRect.y = self.alienRect.height

        # 存储外星人的准确位置
        self.x = float(self.alienRect.x)


    def blitme(self):
        """在指定位置绘制外星人"""

        self.screen.blit(self.image, self.alienRect)
...
def run_game():
    ...
    while True:
        ...
        # 更新屏幕
        game_function.updata_screen(screen,mySettings,ship,bullets,alien)
